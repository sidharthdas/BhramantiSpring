...
public class HolidayPackageDaoImpl implements HolidayPackageDao {
	...
	public ArrayList<HolidayPackageModel> showAllPackages() {
		ArrayList<HolidayPackageModel> holidayPackageModel = (ArrayList<HolidayPackageModel>)getSession().createQuery("FROM HolidayPackageModel").list();
	...
	public HolidayPackageModel showPackageByName(String packageName) {
		...
		ArrayList<HolidayPackageModel> holidayPackageModel	= (ArrayList<HolidayPackageModel>) getSession().createQuery("FROM HolidayPackageModel WHERE packageName = ?").setParameter(0, packageName).list();
	...
	public String addPackage(HolidayPackage holidayPackage) {
		...
		if(checkAmount.equals(true)){
			double discountAmount = discountCalculation(holidayPackage.getPackagePrice(),holidayPackage.getPackageDiscountPercent());
	...
	public String deletePackage(String packageName) {
		...
		String packageDetail = getSession().createQuery("SELECT packageDetail FROM HolidayPackageModel WHERE packageName = ?").setParameter(0, packageName).list().toString();
		System.out.println("packageDetail : "+packageDetail );
		if(packageDetail.equals("[]")){
			return "Package not persent with this name";
		}
		getSession().createQuery("DELETE FROM HolidayPackageModel WHERE packageName = ?").setParameter(0, packageName).executeUpdate();
